...
// From: arxiv-base-develop\arxiv\taxonomy\definitions.py
...
type Categories struct {
	name2category map[string]*Category
}

func (p *Categories) Init() {
	p.name2category = map[string]*Category{
		"acc-phys": {
		...
		"astro-ph.CO": {
			...
			in_archive: "astro-ph",
			description: "Phenomenology of early universe, cosmic microwave " + "background, cosmological parameters, primordial " +
				"element abundances, extragalactic distance scale, " + "large-scale structure of the universe. Groups, " +
				"superclusters, voids, intergalactic medium. Particle " + "astrophysics: dark energy, dark matter, baryogenesis, " +
		...
		"stat.TH": {
			name:       "Statistics Theory",
			in_archive: "stat",
			description: "stat.TH is an alias for math.ST. Asymptotics, " + "Bayesian Inference, Decision Theory, Estimation, " +
...
func (p *Categories) Find(name string) (*Category, bool) {
	item, exists := p.name2category[name]
	return item, exists
}
...
type Groups struct {
	name2group map[string]*Group
}

func (p *Groups) Init() {
	p.name2group = map[string]*Group{
		"grp_physics": {
			name:            "Physics",
			start_year:      1991,
...
func (p *Groups) Find(name string) (*Group, bool) {
	item, exists := p.name2group[name]
	return item, exists
}
...
type Archives struct {
	name2archive map[string]*Archive
}

func (p *Archives) Init() {
	p.name2archive = map[string]*Archive{
		"acc-phys": {
			name:       "Accelerator Physics",
...
func (p *Archives) Find(name string) (*Archive, bool) {
	item, exists := p.name2archive[name]
	return item, exists
}
...
type Subsumed struct {
	archive2subsumed map[string]string
}

func (p *Subsumed) Init() {
	p.archive2subsumed = map[string]string{
		"cmp-lg":   "cs.CL",
		"adap-org": "nlin.AO",
		"comp-gas": "nlin.CG",
		"chao-dyn": "nlin.CD",
...
type CategoryFinder struct {
	...
	arxivGroups     Groups
	arxivArchives   Archives
	arxivSubsumed   Subsumed
}

func (p *CategoryFinder) Init() {
	p.arxivArchives.Init()
	p.arxivCategories.Init()
	p.arxivGroups.Init()
...
func (p *CategoryFinder) FindCategory2(name string) {
	...
	if exists {
		...
		if ex2 {
			...
			if ex3 {
				fmt.Printf("%s|%s|%s|%s\n", name, detailname, arc.name, grp.name)
...
func findAll() {
	...
	var cf CategoryFinder
	cf.Init()

	for _, line := range lines {
		cf.FindCategory2(line)
		//fmt.Println("=======================")
	}
}
